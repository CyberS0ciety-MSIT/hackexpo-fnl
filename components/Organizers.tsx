    ...
    <section className="py-20 md:py-32 bg-nothing-black relative border-b border-nothing-border overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-stretch justify-center gap-8 lg:gap-16 max-w-5xl mx-auto">
          ...
          {/* Organization 1: MSIT KOLKATA - Blue/Cyan Theme */}
          ...
          {/* Organization 2: CYBER SOCIETY - Purple/Gold Theme */}
          ...
          {/* Center Connector 2 */}
          <ScrollReveal delay={500} className="self-center py-4 md:py-0">
            <div className="relative w-16 h-16 flex items-center justify-center group">
              <div className="absolute inset-0 bg-gradient-to-r from-purple-500 to-yellow-500 rounded-full blur-xl opacity-20 group-hover:opacity-40 transition-opacity duration-500"></div>
              <span className="font-mono text-2xl text-white/40 font-bold group-hover:text-white transition-colors">X</span>
          ...
          {/* Organization 3: CRAC LEARNING - Gold/Maroon Theme */}
          <ScrollReveal delay={600} className="flex-1">
            <a
              href="https://www.crac-learning.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="group h-full relative bg-nothing-card/40 border border-nothing-border hover:border-yellow-500/50 transition-all duration-500 p-8 md:p-10 flex flex-col items-center text-center overflow-hidden rounded-xl hover:shadow-[0_0_50px_-20px_rgba(234,179,8,0.15)] backdrop-blur-sm block cursor-pointer"
            >
              <div className="absolute inset-0 bg-[linear-gradient(135deg,transparent_25%,rgba(234,179,8,0.03)_50%,transparent_75%)] bg-[length:250%_250%] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"></div>

              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <ExternalLink className="w-4 h-4 text-yellow-400" />
              </div>

              <div className="relative w-40 h-40 md:w-48 md:h-48 mb-8 flex items-center justify-center p-6 bg-black/40 rounded-full border border-white/10 group-hover:scale-105 transition-transform duration-500 group-hover:border-yellow-500/30">
                <div className="absolute inset-0 border border-dashed border-yellow-500/20 rounded-full animate-spin-slow opacity-0 group-hover:opacity-100 transition-opacity"></div>

                <div className="relative z-10 w-full h-full p-2 flex items-center justify-center">
                  <img
                    src={CRAC_LEARNING_LOGO}
                    alt="CRAC Learning Logo"
                    className="w-full h-full object-contain filter drop-shadow-[0_0_8px_rgba(234,179,8,0.5)] opacity-90 group-hover:opacity-100 transition-opacity"
                  />
                </div>
              </div>

              <div className="relative z-10">
                <h4 className="text-2xl md:text-3xl font-bold text-white mb-2 group-hover:text-yellow-400 transition-colors tracking-tight font-sans">
                  CRAC<br />LEARNING
                </h4>
                <div className="w-12 h-1 bg-yellow-500/30 mx-auto my-4 group-hover:w-24 transition-all duration-500 rounded-full"></div>
              </div>

              <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-yellow-500/30 group-hover:border-yellow-500 transition-colors rounded-tr-lg"></div>
              <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-yellow-500/30 group-hover:border-yellow-500 transition-colors rounded-bl-lg"></div>
            </a>
          </ScrollReveal>
